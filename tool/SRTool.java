...
//		 String filename = "tool/del.c";
		...
		// for (VarDeclContext vdc:ctx.globals){
		// System.out.println(vdc.getText());
		// }
		VCGenerator vcgen = new VCGenerator(ctx);
		vcgen.getGlobalResult();

		assert ctx.procedures.size() == 1; // For Part 1 of the coursework, this
											// can be assumed
		for (ProcedureDeclContext proc : ctx.procedures) {// global var
			vcgen = new VCGenerator(proc);
